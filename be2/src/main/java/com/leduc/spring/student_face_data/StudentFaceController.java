...
public class StudentFaceController {
    ...
    @Operation(summary = "Đăng ký ảnh khuôn mặt sinh viên", description = "Đăng ký 5 ảnh khuôn mặt (trên, dưới, trái, phải, giữa) cho sinh viên hiện tại")
    ...
    @Operation(summary = "Xoá khuôn mặt sinh viên", description = "Xoá một faceId khỏi collection")
    ...
    @Operation(summary = "So sánh khuôn mặt", description = "So sánh 1 ảnh khuôn mặt với collection để xác minh sinh viên")
    ...
    @Operation(summary = "Tạo session cho Face Liveness", description = "Tạo một session mới để kiểm tra liveness của khuôn mặt")
    public ResponseEntity<ApiResponse<LivenessSessionResponse>> createLivenessSession(HttpServletRequest servletRequest) {
        // Gọi service trả về DTO của riêng chúng ta
