...
import { apiStudentService, Student } from '../../api/apiStudent'; // Import apiStudentService and Student interface

const StudentListPage = () => {
  const navigation = useNavigation();
  const route = useRoute();
  const { className, scheduleId, date } = route.params as { className: string; scheduleId: string; date: string }; // Get params from navigation

  const [students, setStudents] = useState<Student[]>([]); // Use Student interface for type safety
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      try {
        ...
        apiStudentService.setAuthToken(token); // Use apiStudentService for token
        const fetchedStudents = await apiStudentService.getStudentsByClass(className); // Correct API call
        
        // Transform data to match existing student card structure (already compatible with Student interface)
